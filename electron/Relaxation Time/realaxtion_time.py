...
def set_plot_style():
    """Set plot style for better visualization (fix minus sign display issue)"""
...
class GraphEnergyDataset(Dataset):
    ...
    def _load_energy_and_fit_tau(self):
        ...
        start_en_num = 30  # Corresponding to size=20
        end_en_num = 100   # Corresponding to size=100
        
        for en_num in range(start_en_num, end_en_num + 1):
            # Removed: Skip even energy index logic, keep all energy indices (both even and odd)
            size = en_num - 1
    ...
    def _load_shared_full_hamiltonians(self):
        ...
        hamiltonian_size = 100  # Must match actual Hamiltonian size
...
def plot_training_history(history, save_path):
    """Plot training history (loss and R² curves)"""
...
def plot_single_tau_visualization(size_list, mean_true_list, mean_pred_list, 
    ...
    """Plot τ_D prediction scatter plot for single dataset (train/test)"""
    set_plot_style()  # Set plot style
...
def plot_combined_tau_visualization(train_data, test_data, save_dir):
    """Plot combined τ_D prediction scatter plot for train+test sets (comparative display)"""
    set_plot_style()  # Set plot style
    ...
    plt.tick_params(axis='both', labelsize=20)  # Adjust X and Y axis simultaneously
...
def predict_all_taus(model, dataset, dataset_type, device, save_dir):
    ...
    # 6. Plot single dataset scatter plot
...
def regenerate_graph_dataset(train_energy_count=7):
    ...
    Data splitting logic remains consistent with previous version: split by energy index, training set contains min and max energy indices, customize training set energy index count
    """
    config = {
    ...
    # Split dataset by energy index (keep original logic)
    ...
    if len(all_energy_nums) < train_energy_count:
        raise ValueError(f"Insufficient available energy indices! Found {len(all_energy_nums)}, need {train_energy_count} for training set")
    ...
    print(f"Min energy index: {min_en}, Max energy index: {max_en}")
    ...
    for size in full_dataset.valid_sizes:
        en_num = full_dataset.size_to_meta[size]["energy_num"]
        count = full_dataset.ham_count_per_group 
