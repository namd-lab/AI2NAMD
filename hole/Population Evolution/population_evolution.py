...
class GraphEnergyDataset(Dataset):
    def __init__(self, config, ham_count_per_group=400, load_initial_from_csv=False, initial_csv_path=None):
        ...
        # Key modification: Predefine large matrix size to ensure access when loading energy data
            ...
            for size in self.valid_sizes:
                for curve in self.energy_data[size]:
                    if len(curve) != self.energy_curve_len:
                        raise ValueError(f"Inconsistent Energy curve length! Size={size}某curve length is {len(curve)}, standard is {self.energy_curve_len}")
        else:
    ...
    def _save_initial_to_csv(self, initial_data):
        ...
        print(f"✅ Initial energy saved successfully: {self.initial_save_csv} (Total {len(df)} samples)")
    ...
    def _load_energy_and_save_initial_csv(self):
        ...
        for en_num in range(start_en_num, end_en_num + 1):
            ...
            if size <= 0:
                print(f"⚠️ Skipping en_num={en_num}: Submatrix size is {size} (invalid)")
                continue
...
# -------------------------- 5. Evaluation and Visualization Utility Functions --------------------------
...
def collect_mean_r2_results(dataset, dataset_type, model, device, save_dir):
    ...
    print(f"\n=== Starting to collect {dataset_type.upper()} set average R² results ===")
    for size in all_sizes:
        size_indices = [i for i in range(len(dataset)) if dataset[i][4] == size]
        if not size_indices:
            print(f"⚠️ Skipping size={size}×{size} (no data)")
...
# -------------------------- 7. Average Encoded Feature Extraction --------------------------
...
def regenerate_graph_dataset(save_initial=True, train_energy_count=10):
    config = {
        ...
        "res_target_pop": "./res_target_pop",  # Energy data directory (jj-enX.dat)
        "ham_count_per_group": 1001,  # Number of Hamiltonian samples per group
        "save_dir": "./pop_shi_mo_data",  # Data save directory
        "train_energy_count": train_energy_count,  # Number of en_nums in training set
        "random_state": 42
    }
    ...
    if len(all_energy_nums) < config["train_energy_count"]:
        raise ValueError(f"Insufficient available energy indices! Current {len(all_energy_nums)}, need {config['train_energy_count']} for training set")
...
def main_training(train_energy_count=10):
    config = {
        "processed_data_dir": "./pop_shi_mo_data",
        "model_save_dir": "./pop_shi_mo_models",
        "prediction_save_dir": "./pop_Si_QD_hole_predictions",
        "features_save_dir": "./pop_Si_QD_hole_features",
        "batch_size": 32,
        "hidden_dim": 32,
        "dropout": 0.2,
        "learning_rate": 3e-4,
    ...
    # Average encoded feature extraction
    print("\n=== Extracting and Saving Mean Encoded Features ===")
    print("\n--- Training Set Mean Encoded Feature Extraction ---")
    ...
    print("\n--- Test Set Mean Encoded Feature Extraction ---")
